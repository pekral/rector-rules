...
render_issue() {
  ...
  printf '%s' "$json" | jq -r --arg mode "$mode" '
    ...
    line("Components"; (.components // [] | join(", "))),
    line("URL"; .url),
    "",
    (if (.descriptionText // "") == "" then "_No description._"
     else "### Description\n\n" + .descriptionText end),
    "",
    ( if $mode != "full" then empty else
          ...
          | "### Comments (\($c | length))",
            ...
            ( if ($c | length) == 0 then "_No comments._"
                     ...
                     | "#### [\(.key)] \(.value.author // "?") — \(.value.created // "?")"
                       + (if (.value.visibility // "public") != "public" then " _(not public)_" else "" end)
                       + "\n\n" + (.value.body // "") )
              end ) ),
        "",
        ( (.attachments // []) as $a
          | "### Attachments (\($a | length))",
            "",
            ( if ($a | length) == 0 then "_No attachments._"
              else ( $a[]
        ...
        ( (.subtasks // []) as $s
          | if ($s | length) == 0 then empty
            else ( "### Subtasks", "", ($s[] | "- \(.key) — \(.summary // "") [\(.status // "?")]") )
            end ),
        ( (.issueLinks // []) as $l
          | if ($l | length) == 0 then empty
            else ( "", "### Issue links", "",
                   ($l[] | "- \(.verb // .type // "relates to") **\(.linkedKey)** — \(.linkedSummary // "") [\(.linkedStatus // "?")]") )
            end ),
        ( (.pullRequests // []) as $p
...
# --- Emit Markdown brief ------------------------------------------------------
echo "# JIRA context: ${ROOT_KEY} — ${ROOT_SUMMARY}"
echo
echo "> Generated by \`gather-issue-context.sh\` as context for working on this task (depth=${DEPTH}, max=${MAX_ISSUES}). Issues loaded: ${#SEEN[@]}."
echo
...
if [[ -n "$URL_INVENTORY" ]]; then
  echo "External links found in the descriptions and comments. The agent loads their content with its own web tools (recursively where needed), following the security rules for outbound requests:"
  echo
  printf '%s\n' "$URL_INVENTORY" | sed 's/^/- /'
else
...
echo "- **Attachment content** is not in this document — \`acli\` does not download attachments. Fetch them safely with \`skills/code-review-jira/scripts/download-attachments.sh <KEY|URL>\` (TLS on, into quarantine) and open **only** the files the security gate (\`scan-attachments.sh\`) promoted into \`safe/\`."
echo "- **The external links** above: follow them with your own web tools; when they lead to further relevant sources, continue recursively to a reasonable depth."
echo "- Linked JIRA issues are loaded to depth ${DEPTH}; raise \`JIRA_CONTEXT_DEPTH\` for deeper context."
