...
_This comment is generated by the {{pr-summary}} skill for non-technical readers. The JIRA publisher converts this source to Atlassian Document Format so headings, emphasis, lists, code, and links render in the JIRA UI._
...
{embedded_blocks}
  Render this slot only when the calling CR wrapper passes one or more supported
  JIRA source blocks — the "h2. Clarifying questions" block (open questions the
  ...
  and Critical items). Each block already uses the intermediate format; append it
