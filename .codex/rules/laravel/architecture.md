...
- **Action-to-Action pass-through rule (Action pattern):** The same rule applies when the single delegated call targets **another Action** — an `__invoke()` whose entire body is `($this->otherAction)($payload)` and nothing else. An Action that composes *several* collaborators, one of them an Action an entry point also calls, is the pattern working as intended (see **Action scope: concrete use case, not general logic**, and *An Action is the application's entry-point contract* for the inner Action nothing outside the Action layer calls).
An Action that only forwards to one other Action is two names for one use case, and the outer one holds no logic at all. Unlike a Service method, an Action is a use case rather than a reusable method, so the resolution is always to **collapse the two into one**:
...
Actions are what the outside world calls. A controller, a job, a console command, a listener, and a Livewire component are the consumers; each Action presents one whole use case to them — `CreateOrderAction`, never the steps that use case runs through.

An Action whose only callers are other Actions is not a use case. It is a private step of the caller's flow wearing the name of a public one, and it turns `app/Actions/**` from the list of things the application does into a list of its internals.

- **The test is the caller, not the size of the class.** Search the whole project source for the Action's call sites. When every non-test caller is another Action, the class is the finding. A call from a test never counts as a caller.
- **The fix is never a second Action.** Return the step to the Action that needs it, or move it to the layer that owns the concern: a Data Validator for a check, a Data Builder for a mapping, a Repository for a read, a ModelManager for a write, a Model Service for a single-model operation a second flow already calls (see **Model Services** → *Avoid introducing a new Model Service — an Action is the default home*). A step only one Action runs, and that no layer owns, stays a private method of that Action.
- **An Action an entry point also calls may be composed freely.** `CreateOrderAction` invoking `SendOrderConfirmationAction` is two use cases composed, not one use case split. The inner Action exists on its own terms, because a controller, a job, or a command calls it too. This rule fires only on the inner Action that nothing outside the Action layer ever calls.
- **Faulty example.** `ClearOrderCacheAction`, injected into `CreateOrderAction` and invoked from nowhere else. Its name promises a use case a consumer can ask for, and no consumer can.
- **The cost this rule accepts, stated rather than hidden.** Extracting that class out of `CreateOrderAction` reads as the single-responsibility principle, and here it is refused. The split itself still happens — into the layer that owns the concern, never into a second Action. What the rule buys is that the Action folder keeps describing the application's outside surface.
...
- **An Action is the orchestration of one concrete use case — it composes several collaborators (Model Services, Data Validators, Data Builders, and Actions an entry point also calls) into a single business flow.** A composed Action is a use case in its own right, never a step extracted out of the caller (see *An Action is the application's entry-point contract, never a step inside another Action*). The reference *orchestration* shape is `pekral/arch-app-services` `examples/Actions/User/CreateUser.php`: it coordinates validation, normalization, persistence via a Model Service, and a follow-up Action — the Action *coordinates* the steps, it does not implement them. (Note: that package example wires validation and mapping through the in-Action `DataValidator` / `DataBuilder` traits;
...
- **When the project has `spatie/laravel-data`, every DTO extends `Spatie\LaravelData\Data`.** One base class per project, not a mix of `Data` subclasses and hand-rolled carriers that re-implement what the base class already gives: `::from(...)`, `->toArray()`, `->all()`, collection casting, and property mapping. A hand-written `fromArray()` / `toArray()` pair on a DTO is a smell — delete it and let the base class carry it.
- **Never declare a `Data` subclass as a `readonly class` — PHP rejects it at load time, not at review time.** `Data` is not a readonly class, and PHP forbids a readonly class from extending a non-readonly one: `Readonly class Foo cannot extend non-readonly class Spatie\LaravelData\Data`. Every fixer, every analyser, and every code review stays green, and the failure lands in whichever request or job first touches the DTO. Carry immutability on the **properties** instead — mark each promoted constructor property `public readonly`, and a write then throws `Cannot modify readonly property`. This is the one place where the *Make a data-carrying class immutable (`final readonly`)* default in `@rules/php/core-standards.md` Structure does not apply: `final` stays, class-level `readonly` goes.
- **The fix for a non-`Data` DTO is `extends Data` where the class already lives.** Moving it out of `app/Dto/**` relocates the same class and leaves the next reader a DTO-shaped object in a namespace that promises something else. The narrow exception is a genuine **value object** — a type whose own construction invariant is the point (a private constructor plus a named factory) — which does not belong under `app/Dto/**` in the first place and moves to a namespace that states what it is. A parameter object, a context object, or any other bag of values passed between methods is **not** covered by that exception, however local it is: a carrier read by three private methods of one class is still a DTO.
...
- DTO classes should be `final` with promoted constructor properties where practical, and immutable. Declare them `final readonly` when they extend nothing; when they extend `Spatie\LaravelData\Data`, keep `final` and mark the properties `public readonly` instead, per the bullet above.
...
- **The suffix is exclusive: a class whose job is to assemble data carries it, and no other name.** A data-building class named `*Report`, `*Formatter`, `*Factory`, `*Presenter`, `*Mapper`, `*Composer`, or `*Transformer` is the same layer under a name that says nothing about what the class is allowed to do, and it is a finding whichever of those it picks. The fix is the rename plus the move, not a second convention.
- **Split by what is being built, never by who consumes it.** One Data Builder per data structure or destination shape. A class that starts building two unrelated payloads has two reasons to change and becomes the grab bag nobody dares touch — split it at that moment, not later.
...
- **Name every rule trait with the `ValidationRules` suffix.** `SmartDeliveryValidationRules`, `ContactChangeTriggerValidationRules`. A rule set placed inline in a FormRequest, or parked under a `Support` / `Http\Requests\Traits` folder, is the same logic in a home nothing else looks in.
- **Extract on the second consumer, not the third.** As soon as the same rule array is needed by a second request class — the single and the bulk variant of one endpoint are the usual pair — it moves into `app/Concerns/` and both `use` it. Copying the array is what this rule exists to stop.
- **Parameterise the trait by the attribute path instead of forking it.** The method takes the path as an argument (`firebasePushRules('subscriber_data')` / `firebasePushRules('subscriber_data.*')`), so one trait serves the single and the bulk surface. A second near-identical trait per surface is a finding.
- Use custom Rule classes in `app/Rules/` with the `Rule` suffix for complex or reusable validation logic that goes beyond simple rule arrays. The boundary is composition versus unit: `app/Concerns/` holds the **array of rules** for a group of attributes, `app/Rules/` holds the **individual rule objects** that array references.
...
- **A Data Validator returns `bool` or throws an exception — nothing else.** Every public method of a Data Validator has one of two shapes:
  - a predicate that returns `bool` and answers whether the input satisfies the condition (e.g. `evaluate(ContactChangeCondition $condition, ChangeModel $change): bool`);
  - an assertion that returns `void` and throws an exception when the input is invalid — `ValidationException`, or a typed domain exception (e.g. `assertPayable(OrderModel $order, PaymentModel $payment): void`).
...
The controller converts it first — `$request->toDto()`, or `$request->validated()` mapped by a Data Builder — and passes the DTO (see **Action Rules** → *accept the HTTP request*). Concrete values pulled off the request (an `UploadedFile`, a route-bound Model, the authenticated user, a scalar) are not the request and never trigger this finding.
  ...
  - a DTO declared as a `readonly class` while extending `Spatie\LaravelData\Data` — PHP refuses to load the class (`Readonly class … cannot extend non-readonly class Spatie\LaravelData\Data`), so the failure reaches production past every fixer, analyser, and review. Keep `final`, drop the class-level `readonly`, and mark each promoted constructor property `public readonly` (see **DTOs**)
...
framework accessors (`user()`, `ip()`, `method()` / `isMethod()`, `fullUrl()` / `url()` / `path()`, `route()`, `bearerToken()`, `session()`, `expectsJson()`) do not by themselves trigger this finding — see **Controllers and Other Entry Points** for the full exempt list. Every request shape must have its own dedicated FormRequest that owns validation rules, authorization, and input shaping.
  ...
  - a public Data Validator method whose return type is anything other than `bool` or `void` (it returns validated / normalized data, a DTO, a model, a `Collection`, error messages, or a nullable result encoding *invalid*) — the method must return `bool` or throw (see **Data Validators** → *A Data Validator returns `bool` or throws an exception*)
...
**Gating — one finding per class, never two:** a new Service that does not extend `BaseModelService` is the Critical finding above instead; a new Service that already extends it and crosses into cross-model orchestration is the Moderate bullet below instead. This bullet owns the new Service's existence, including the compliant single-public-method case the bullet below exempts
  ...
  - an Action whose only non-test callers are other Actions — an Action is the use case an entry point invokes, never a step inside another Action's flow (see **Actions** → *An Action is the application's entry-point contract, never a step inside another Action*). The **Suggested Fix** deletes the inner Action and returns its body to the calling Action, or moves it to the layer that owns the concern — a Data Validator, a Data Builder, a Repository, a ModelManager, or a Model Service the bullet above already gates.
An Action a controller, job, command, listener, or Livewire component also calls is not this finding, however many other Actions compose it. **Gating — one finding per Action, never two:** when the calling Action's **entire** `__invoke()` body is that single delegating call, the pass-through bullet below owns it instead; this bullet owns the inner Action's existence
  ...
  - a DTO under `app/Dto/**` that does not extend `Spatie\LaravelData\Data` on a project that installs `spatie/laravel-data`, or that hand-rolls a `fromArray()` / `toArray()` pair the base class already provides — the fix is `extends Data` where the class already lives plus the deleted boilerplate, never a move out of the DTO tree (see **DTOs**). A genuine value object whose own construction invariant is the point is exempt and moves to a namespace that states what it is
  - a data-building class carrying a suffix other than `DataBuilder` — `*Report`, `*Formatter`, `*Factory`, `*Presenter`, `*Mapper`, `*Composer`, `*Transformer` — or one Data Builder assembling two unrelated payloads instead of one per built shape (see **Data Builders**). **Gating — one finding per class, never two:** when `pekral/arch-app-services` is installed, the Critical *ad-hoc class hosting business logic* bullet above already owns the misnamed class and this Moderate one does not fire; this bullet is the owner only on a project without the package
  - a reusable validation rule set that is not a `ValidationRules`-suffixed trait in `app/Concerns/` — inlined in a FormRequest, duplicated across a second request class, or parked outside that namespace — and a trait forked per attribute surface instead of taking the attribute path as an argument (see **Validation Rules (Traits)**)
